...
from text2vec.utils.choose_device import get_device_id
# device = torch.device(f"cuda:{get_device_id()}" if torch.cuda.is_available() else "cpu")
...
class SentenceModel:
    def __init__(
        ...
        print('AUTOTOKENIZER,AUTOMODEL')
        # self.tokenizer = BertTokenizer.from_pretrained(model_name_or_path)
        # self.bert = BertModel.from_pretrained(model_name_or_path)
        # print('BERTTOKENIZER,BERTMODEL')
        self.bert.to(device)
