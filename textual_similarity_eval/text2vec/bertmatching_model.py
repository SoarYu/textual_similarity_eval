...
from text2vec.bertmatching_dataset import BertMatchingTestDataset, BertMatchingTrainDataset, \
    load_test_data, load_train_data, HFBertMatchingTrainDataset, HFBertMatchingTestDataset, HFBertMatchingPredictDataset, SCBertMatchingPredictDataset
...
class BertMatchModel:
    def __init__(
        ...
        self.task_name = task_name
        self.model = BertMatchModule(model_name_or_path, num_classes,task_name)
    ...
    def evaluate(self, eval_dataset, output_dir: str = None, batch_size: int = 16):
        ...
        for batch in tqdm(eval_dataloader, disable=False, desc="Running Evaluation"):
            ...
            for p in probs.cpu().numpy():
                # data = np.dot(p, range(len(p))).astype(np.float32)
    ...
    def predict(self, test_dataset, batch_size: int = 16, task="STS-B"):
        """
        Predicts on test_dataset.
        """
        if task == 'SCHOLAT':
            test_dataset_hf = SCBertMatchingPredictDataset(self.tokenizer,self.task_name,max_len=self.max_seq_length)
        else:
            test_dataset_hf = HFBertMatchingPredictDataset(self.tokenizer, task, max_len=self.max_seq_length)
        test_dataloader = DataLoader(test_dataset_hf, batch_size=batch_size)
        self.model.bert.to(device)
        self.model.bert.eval()
        batch_labels = []
        batch_preds = []
        for batch in tqdm(test_dataloader, disable=False, desc="Running Prediction"):
            inputs, labels = batch
            labels = labels.to(device)
            batch_labels.extend(labels.cpu().numpy())
            # inputs        [batch, 1, seq_len] -> [batch, seq_len]
